...
with tab1:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name='크다', x=overview_data['size'], y=overview_data['크다'], marker_color='#ef4444'))
    fig.add_trace(go.Bar(name='적당하다', x=overview_data['size'], y=overview_data['적당하다'], marker_color='#10b981'))
    fig.add_trace(go.Bar(name='작다', x=overview_data['size'], y=overview_data['작다'], marker_color='#3b82f6'))
...
with tab2:
    ...
    with size_tab1:
        ...
        with col1:
            ...
            st.plotly_chart(fig_105)
        with col2:
            st.markdown("### 📊 응답 통계")
            df_105 = size_105.assign(비율=lambda x: (x['인원']/60*100).round(1).astype(str) + '%')
            st.dataframe(df_105)
    ...
    with size_tab2:
        ...
        with col1:
            ...
            st.plotly_chart(fig_110)
        with col2:
            st.markdown("### 📊 응답 통계")
            df_110 = size_110.assign(비율=lambda x: (x['인원']/60*100).round(1).astype(str) + '%')
            st.dataframe(df_110)
    ...
    with size_tab3:
        ...
        with col1:
            ...
            st.plotly_chart(fig_120)
        with col2:
            st.markdown("### 📊 응답 통계")
            df_120 = size_120.assign(비율=lambda x: (x['인원']/60*100).round(1).astype(str) + '%')
            st.dataframe(df_120)
...
with tab4:
    ...
    for i, feedback in enumerate(feedback_105):
        if i % 2 == 0:
            with col1: st.info(f"**{i+1}.** {feedback}")
        else:
            with col2: st.info(f"**{i+1}.** {feedback}")
    ...
    for i, feedback in enumerate(feedback_120):
        if i % 2 == 0:
            with col1: st.success(f"**{i+1}.** {feedback}")
        else:
            with col2: st.success(f"**{i+1}.** {feedback}")

    st.divider()
    st.subheader("🎯 벤치마킹 권장 브랜드")
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.markdown("### 캉골\n**(100 사이즈)**")
    with col2: st.markdown("### 블루독")
    with col3: st.markdown("### 노스키즈")
    with col4: st.markdown("### 뉴발")
